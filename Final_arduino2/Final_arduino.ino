...
void loop(){
  ...
  if (Serial.available() > 0) { // Check if there's a recieved data from Python code
    char received = Serial.read(); // Read the incoming data

    // If you received 1 , start the motor
    if (received == '1') {
      digitalWrite(motor_en, HIGH); 
      digitalWrite(motor_1, HIGH);  // Set motor direction
      digitalWrite(motor_2, LOW);
      Serial.println("Motor ON"); 
    }
    // if you received 0 , stop the motor
    else if (received == '0') {
